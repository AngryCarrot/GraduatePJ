...
def get_input_file_single(request):
    ...
    if request.method == "POST":
        ...
        shutil.move(file_path, osp.join(osp.join(osp.join(settings.BASE_DIR, "ImagesDB"), label), filename))
    return JsonResponse(response)
...
def get_input_file_multiple(request):
    ...
    if request.method == "POST":
        ...
        file_path = osp.join(osp.join(settings.BASE_DIR, "media"), file_name)
        write2disk(file_path, file_data)
